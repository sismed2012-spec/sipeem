...
import {
  commitHistorialSeccion2024Import,
  parseHistorialSeccion2024XLSX,
  previewHistorialSeccion2024Import,
} from "@/actions/import-secciones";
import type {
...
export default function HistorialSeccionImportDialog() {
  ...
  const checksumRows = previewRows.filter((row) => row.status === "omitido").length;
  ...
  async function handleFileUpload(
    ...
    try {
      ...
      const { rows, globalErrors } = await parseHistorialSeccion2024XLSX(
        formData
      );
      ...
      const enrichedRows = await previewHistorialSeccion2024Import(rows);
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Error al procesar el XLSX seccional"
      );
    } finally {
      setLoading(false);
    }
  ...
  async function handleCommit() {
    setLoading(true);
    try {
      const response = await commitHistorialSeccion2024Import(previewRows);
      setResult(response);
      setStep(3);
      toast.success(
        `Carga seccional completada: ${response.inserted} creados, ${response.updated} actualizados`
      );
      router.refresh();
    } catch (error) {
      toast.error(
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden flex flex-col p-0 rounded-2xl border-none shadow-2xl">
        <DialogHeader className="p-6 bg-slate-50 border-b border-slate-100">
          <DialogTitle className="text-2xl font-black text-slate-900 tracking-tighter uppercase">
            {step === 1 && "Importar Historico 2024 por Seccion"}
            {step === 2 && "Previsualizacion de Carga Seccional"}
            {step === 3 && "Resumen de Carga Seccional"}
          </DialogTitle>
          <DialogDescription className="font-medium text-slate-500">
            {step === 1 &&
              "Sube el XLSX oficial por seccion. Se validan municipios, secciones, checksums y totales antes de persistir."}
            {step === 2 &&
              "Revisa el mapeo contra el catalogo interno. Las filas con SECCION 0 se marcan como checksum y no se guardan."}
            {step === 3 &&
              "Resumen final de filas insertadas, actualizadas y omitidas durante la carga seccional."}
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-6">
              <div className="grid gap-4 md:grid-cols-3">
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  <p className="mt-2 text-sm font-bold text-slate-900">
                    XLSX ancho oficial por seccion
                  </p>
              ...
              <div className="flex flex-col items-center justify-center p-12 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50 hover:bg-slate-50 transition-colors group cursor-pointer relative">
                ...
                <p className="font-black text-slate-900 uppercase tracking-widest text-sm text-center">
                  Arrastre o seleccione su XLSX 2024
                </p>
