# Compromisos de Campaña — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Tab "Compromisos" en la ficha municipal para registrar y dar seguimiento a promesas de campaña por tema (obra, servicio, gestión) con estado de cumplimiento.

**Architecture:** Tabla `compromisos_campana` + actions en `src/actions/compromisos.ts` + panel `CompromisosPanel` con CRUD y cambio de estatus inline. Tab en `ActoresTabs`.
...
- Patrón de referencia: `IncidenciasPanel` de A4 (tiene cambio de estatus inline)
- Este módulo agrega el tab #9 a ActoresTabs (o #7/8 dependiendo del orden de A2/A4)

## File Map

| Archivo | Acción |
|---------|--------|
| `src/actions/compromisos.ts` | Crear |
| `src/components/actores/CompromisosPanel.tsx` | Crear |
| `src/lib/types.ts` | Modificar — agregar `CompromisoCampana` |
| `src/actions/actores.ts` | Modificar — agregar `compromisos` a `ActoresMunicipioData` |
| `src/components/actores/ActoresTabs.tsx` | Modificar — agregar tab |

---

### Task 1: SQL tabla `compromisos_campana`

- [ ] **Step 1: Ejecutar en Supabase SQL Editor**

```sql
CREATE TABLE public.compromisos_campana (
  id                  bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  municipio_id        bigint NOT NULL REFERENCES public.municipios(id) ON DELETE CASCADE,
  titulo              text NOT NULL,
  descripcion         text,
  tema                text NOT NULL CHECK (tema IN ('obra','servicio','gestion','social','seguridad','otro')),
  estatus             text NOT NULL DEFAULT 'pendiente' CHECK (estatus IN ('pendiente','en_proceso','cumplido','cancelado')),
...
CREATE INDEX compromisos_campana_municipio_idx ON public.compromisos_campana (municipio_id);
```

---

### Task 2: Tipo `CompromisoCampana` en types.ts
...
**Files:**
- Create: `src/actions/compromisos.ts`

- [ ] **Step 1: Crear el archivo**

```typescript
// src/actions/compromisos.ts
"use server";
...
### Task 4: Componente `CompromisosPanel`

**Files:**
- Create: `src/components/actores/CompromisosPanel.tsx`

- [ ] **Step 1: Crear componente**

```tsx
// src/components/actores/CompromisosPanel.tsx
"use client";
...
- [ ] **Step 1: Actualizar `ActoresMunicipioData` en `src/actions/actores.ts`**
...
// Actualizar tipo:
compromisos: CompromisoCampana[];  // NUEVO

// En getActoresMunicipio, agregar:
...
1. `import CompromisosPanel from "./CompromisosPanel"`
2. Agregar `compromisos: CompromisoCampana[]` al tipo de actores
3. Agregar `<TabsTrigger value="compromisos">Compromisos</TabsTrigger>`
4. Agregar `<TabsContent value="compromisos"><CompromisosPanel municipioId={municipioId} initialCompromisos={actores.compromisos} /></TabsContent>`
