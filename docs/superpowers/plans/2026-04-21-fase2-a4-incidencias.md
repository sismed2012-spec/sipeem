# Registro de Incidencias — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Tab "Incidencias" en la ficha municipal para registrar eventos adversos (violencia política, acarreo, compra de voto) con severidad y seguimiento de resolución.

**Architecture:** Tabla `incidencias` + server actions en `src/actions/incidencias.ts` + panel `IncidenciasPanel` con CRUD (add + edición de estatus + delete). Tab nuevo en `ActoresTabs`.
...
- Patrón de referencia: `src/components/actores/AspirantesPanel.tsx` (tiene edición inline)
- Este módulo agrega el tab #8 a ActoresTabs (después de Agenda de A2)
- Si A2 aún no está implementado, agregar directamente como tab #7

## File Map

| Archivo | Acción |
|---------|--------|
| `src/actions/incidencias.ts` | Crear |
| `src/components/actores/IncidenciasPanel.tsx` | Crear |
| `src/lib/types.ts` | Modificar — agregar `Incidencia` |
| `src/actions/actores.ts` | Modificar — agregar `incidencias` a `ActoresMunicipioData` |
| `src/components/actores/ActoresTabs.tsx` | Modificar — agregar tab |

---

### Task 1: SQL tabla `incidencias`
...
CREATE TABLE public.incidencias (
  ...
  tipo          text NOT NULL CHECK (tipo IN ('violencia','acarreo','compra_voto','propaganda_ilegal','otro')),
  descripcion   text NOT NULL,
  severidad     text NOT NULL CHECK (severidad IN ('baja','media','alta','critica')),
  estatus       text NOT NULL DEFAULT 'abierta' CHECK (estatus IN ('abierta','en_seguimiento','resuelta')),
  fecha         date NOT NULL,
  reportado_por text,
  notas         text,
...
CREATE INDEX incidencias_municipio_idx ON public.incidencias (municipio_id);
CREATE INDEX incidencias_fecha_idx ON public.incidencias (fecha DESC);
```

---

### Task 2: Tipo `Incidencia` en types.ts

- [ ] **Step 1: Agregar al final de `src/lib/types.ts`**
...
**Files:**
- Create: `src/actions/incidencias.ts`
...
### Task 4: Componente `IncidenciasPanel`

**Files:**
- Create: `src/components/actores/IncidenciasPanel.tsx`
...
import type { Incidencia } from "@/lib/types";
import {
  createIncidencia, updateIncidenciaEstatus, deleteIncidencia,
          ...
          {initialIncidencias.map((inc) => (
            <div key={inc.id} className="rounded-2xl border border-slate-200 p-4 bg-white space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Select
                  value={inc.estatus}
                  onValueChange={(v) => handleEstatusChange(inc.id, v as Incidencia["estatus"])}
        ...
        <form onSubmit={handleAdd} className="rounded-2xl border border-slate-200 p-4 space-y-4 bg-slate-50">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="grid gap-1.5">
              ...
              <Select value={tipo} onValueChange={(v) => setTipo(v as Incidencia["tipo"])}>
            ...
            <div className="grid gap-1.5">
              ...
              <Select value={severidad} onValueChange={(v) => setSeveridad(v as Incidencia["severidad"])}>
                <SelectTrigger className="rounded-xl border-slate-200 text-sm"><SelectValue /></SelectTrigger>
                <SelectContent>
...
### Task 5: Integrar en ActoresTabs y ActoresMunicipioData

- [ ] **Step 1: Agregar `Incidencia` a `ActoresMunicipioData` en `src/actions/actores.ts`**
...
export type ActoresMunicipioData = {
  ...
  eventos: EventoCampana[];      // de A2 (si ya existe)
  incidencias: Incidencia[];     // NUEVO
};
...
// En el return:
return { termometros, escenarios, comite, planilla, aspirantes, eventos, incidencias };
```

- [ ] **Step 2: Agregar tab en `ActoresTabs.tsx`**

1. Agregar `import IncidenciasPanel from "./IncidenciasPanel"`.
2. En props/type, agregar `incidencias: Incidencia[]` al tipo de `actores`.
3. Agregar `<TabsTrigger value="incidencias">Incidencias</TabsTrigger>`.
4. Agregar `<TabsContent value="incidencias"><IncidenciasPanel municipioId={municipioId} initialIncidencias={actores.incidencias} /></TabsContent>`.
